...
exports.getList = (req, res) => {
    //try {
        ...
        let startFilter = req.query.startFilter || null;
        let endilter = req.query.endilter || null;
        if(search !== null){
            search = moment(search, 'YYYY-MM-DD').format('YYYY-MM-DD');
        }
        if(startFilter !== null && endilter !== null){
            startFilter = moment(startFilter, 'YYYY-MM-DD').format('YYYY-MM-DD');
            endilter = moment(endilter, 'YYYY-MM-DD').format('YYYY-MM-DD');
        }
        productModel.getList((error, results) => {
            if (error) {
                return res.status(500).json({ error: 'Database query error' });
            }
            productModel.countAll((error, resultsCOunt) => {
                if (error) {
                    return res.status(500).json({ error: 'Database query error' });
                }
                return res.json({
...
exports.getLatestRecord = (req, res) => {
